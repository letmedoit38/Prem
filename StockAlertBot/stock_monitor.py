...
class Snapshot:
    ...
    prev_close:        float
    day_chg_pct:       float
    drop_from_high:    float   # % below 52-week high (positive = drop)
...
def fetch_snapshots(symbols: Dict[str, dict]) -> List[Snapshot]:
    ...
    for sym, meta in symbols.items():
        snap = _fetch(sym, meta)
        results.append(snap)
    return sorted(results, key=lambda s: s.symbol)


def _fetch(symbol: str, meta: dict) -> Snapshot:
    base = Snapshot(symbol=symbol, name=meta.get("name", symbol),
                    sector=meta.get("sector", ""), price=0, high_52w=0,
                    low_52w=0, prev_close=0, day_chg_pct=0, drop_from_high=0)
    try:
    ...
    for s in snaps:
        ...
        if s.alert_drop:
            msgs.append(
                ...
                f"⬇️  Drop from High: *{s.drop_from_high:.1f}%*\n"
                f"📆 Today: {_sign(s.day_chg_pct)}{abs(s.day_chg_pct):.2f}%"
            )
...
def make_status_report(snaps: List[Snapshot]) -> str:
    ...
    for s in snaps:
        if s.error:
            ...
            continue
        dot  = "🟢" if s.day_chg_pct >= 0 else "🔴"
        flag = " 🔔" if s.alert_drop else (" ⚠️" if s.alert_near_low else "")
        lines.append(
            f"{dot} `{s.symbol}` ₹{s.price:,.2f} "
            f"\\({_sign(s.day_chg_pct)}{abs(s.day_chg_pct):.1f}%\\)"
